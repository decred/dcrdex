...
type NexusConfig struct {
	...
	// can be generated by deleting the old key-cert pair an restarting.
	// Changing the ExternalAddr renders any previously generated relayfiles
	// void.
	Key string
	// The path to the TLS certificate. Default value is a
	Cert   string
	Logger dex.Logger
...
// normalize checks sanity and set defaults for the NexusConfig.
...
// contain all the relevant connection information for a source nodes
...
func (n *Nexus) handleSourceConnect(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		select {
		case relayID := <-nodeChan:
			node.relayID = relayID
		...
		if _, found := n.relayAddrs[node.relayID]; !found {
			log.Errorf("no nexus relay registered for relay ID %s", node.relayID)
			return
		}
