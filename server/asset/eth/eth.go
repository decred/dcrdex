...
func NewBackend(configPath string, log dex.Logger, net dex.Network) (*ETHBackend, error) {
	...
	bondContractAddress, found := dexeth.ETHBondAddress[eth.net]
	if !found {
		return nil, fmt.Errorf("no bond address for network %s", eth.net)
	}

	eth.node = newRPCClient(net, endpoints, ethContractAddr, bondContractAddress, log.SubLogger("RPC"))
	return eth, nil
}
