...
func (be *AssetBackend) userOpBaseCoin(txHash, userOpHash common.Hash, contractData []byte) (*baseCoin, error) {
	...
	var epAddress common.Address
	var foundVersion bool
	for _, address := range be.entryPointAddresses {
		if address == *tx.To() {
			epAddress = address
			foundVersion = true
			break
	...
	handleOpsData, err := dexeth.ParseV0_6HandleOpsData(tx.Data())
	if err != nil {
		return nil, fmt.Errorf("unable to parse v0_6 handle ops data: %v", err)
	}

	var userOp *v6EP.UserOperation
	for _, op := range handleOpsData {
		hash, err := dexeth.HashV0_6UserOp(op, epAddress, big.NewInt(int64(be.evmChainID)))
		if err != nil {
