...
func TestNewRedeemCoin(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.txErr == nil && (rc.secret != secret ||
			rc.value != 0 ||
...
func TestNewSwapCoin(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			name:    "tx has no data",
			tx:      tTx(gasPrice, gasTipCap, txVal, contractAddr, nil),
			coinID:  txCoinIDBytes,
			locator: secretHashA[:],
			wantErr: true,
		}, {
			name:    "unable to decode init data, must be init for init coin type",
			tx:      tTx(gasPrice, gasTipCap, txVal, contractAddr, redeemCalldataV0),
			coinID:  txCoinIDBytes,
			locator: secretHashA[:],
			wantErr: true,
		}, {
			...
			name:    "wrong contract",
			tx:      tTx(gasPrice, gasTipCap, txVal, randomAddr, initCalldataV0),
			coinID:  txCoinIDBytes,
			locator: secretHashA[:],
			wantErr: true,
		}, {
	...
	for _, test := range tests {
		...
		if sc.vector.To != participantAddr ||
			...
			sc.value != txVal ||
			sc.vector.Value.Cmp(dexeth.GweiToWei(initValue)) != 0 ||
			...
			fmt.Println("value:", sc.value, txVal)
			fmt.Println("swap value:", sc.vector.Value, dexeth.GweiToWei(initValue))
...
// 	dataV1, err := dexeth.ABIs[1].Pack("initiate", inits)
// 	if err != nil {
// 		t.Fatalf("Pack error: %v", err)
// 	}
...
// 		},
// 	}
// 	dataV1, err = dexeth.ABIs[1].Pack("redeem", redeemsV1)
// 	if err != nil {
// 		t.Fatalf("Pack error: %v", err)
// 	}
