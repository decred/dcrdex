...
func TestParseBondTx(t *testing.T) {
	...
	// addr, err := stdaddr.NewAddressPubKeyEcdsaSecp256k1V0(bondPubKey, chainParams)
	// if err != nil {
	// 	t.Fatalf("NewAddressPubKeyEcdsaSecp256k1V0 error: %v\n", err)
	// }
	lockTime := time.Now().Add(time.Hour)
	bondRedeemScript, err := txscript.NewScriptBuilder().
		AddInt64(lockTime.Unix()).
		AddOp(txscript.OP_CHECKLOCKTIMEVERIFY).
		AddOp(txscript.OP_DROP).
		AddData(bondPubKey.SerializeCompressed()).
	...
	gotCoinID, gotAmt, gotAddr, gotPubKey, gotLockTime, gotAcct, err := ParseBondTx(txRaw, bondRedeemScript)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Log(gotCoinID, gotAmt, gotAddr, gotPubKey, gotLockTime, gotAcct)
