...
type Storage interface {
	...
	UpdateAssetBonds(acct *account.Account, bonds []*db.Bond, newAcct bool) error
...
// certain asset, and replaces them with the bonds provided.
func (auth *AuthManager) updateAssetBonds(user account.AccountID, assetID uint32, bonds []*db.Bond) *account.Reputation {
	client := auth.user(user)
	if client == nil {
		return nil // offline
	}
