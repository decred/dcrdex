...
func (auth *AuthManager) handlePostBond(conn comms.Link, msg *msgjson.Message) *msgjson.Error {
	...
	dbAcct, bonds, _, legacyFeePaid := auth.storage.Account(acctID, lockTimeThresh)
	...
	chkAddr, chkAmt, confs, err := auth.checkFee(assetID, bondCoinID)
	if err != nil {
	...
	if chkAddr != bondAddr {
		return msgjson.NewError(msgjson.BondError, "node reports incorrect bond address: %v != %v", chkAddr, bondAddr)
	}

	reqConfs := int64(auth.bondConfs)
	...
	auth.latencyQ.Wait(&wait.Waiter{
		...
		TryFunc: func() wait.TryDirective {
			res := auth.waitBondConfs(conn, dbBond, acctID, assetID, reqConfs)
...
// status on 'connect'.
func (auth *AuthManager) waitBondConfs(conn comms.Link, bond *db.Bond, acctID account.AccountID, assetID uint32, reqConfs int64) wait.TryDirective {
	coinID := bond.CoinID
	addr, val, confs, err := auth.checkFee(assetID, coinID) // e.g. FeeCoin
	if err != nil {
