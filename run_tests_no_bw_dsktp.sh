...
#
# Go mod tidy the awkward ones
#
pushd .
cd $HOME/dex/dcrdex/client/cmd/bisonw-desktop
go mod tidy -v
cd $HOME/dex/dcrdex/dex/testing/loadbot
go mod tidy -v
popd
echo "-------------------------------------- Ran go mod tidy on the twins ---------------------------"
...
for m in $modules
do
	if [ "$m" == "/client/cmd/bisonw-desktop" ] ; then
		cd "$HOME/dex/dcrdex/client/cmd/bisonw-desktop"
		go mod tidy -v
		echo "========================== skipping bisonw-desktop =============================="
		cd -
		echo $(pwd)
	else
	cd "$dir/$m"
