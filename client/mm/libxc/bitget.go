...
func (bitgetBidsComparable) CalcScore(key any) float64 {
	return math.MaxFloat64 - float64(key.(*bitgetObEntry).rate)
}
...
// parseAndConvert parses a string price/qty pair and returns a bitgetObEntry.
// Returns nil if parsing fails or if qty is "0" (removal marker).
func (b *bitgetOrderBook) parseAndConvert(priceStr, qtyStr string) *bitgetObEntry {
	if qtyStr == "0" {
		return nil // Removal marker
	}

	price, err := strconv.ParseFloat(priceStr, 64)
	if err != nil {
		b.log.Warnf("%s: failed to parse price %q: %v", b.symbol, priceStr, err)
		return nil
	}
...
func (b *bitgetOrderBook) update(bids, asks [][]string) {
	...
	for _, u := range bids {
		...
		// Create a lookup key with just the rate for finding existing entries
		price, err := strconv.ParseFloat(priceStr, 64)
		if err != nil {
			continue
		}
		lookupRate := calc.MessageRateAlt(price, b.baseConversionFactor, b.quoteConversionFactor)
		lookupKey := &bitgetObEntry{rate: lookupRate}

		if qtyStr == "0" {
			// Remove entry
			b.bids.Remove(lookupKey)
		} else {
			// Remove existing entry at this rate (if any) and insert new one
			b.bids.Remove(lookupKey)
			if entry := b.parseAndConvert(priceStr, qtyStr); entry != nil {
				b.bids.Set(entry, entry)
			}
		}
	}
	...
	for _, u := range asks {
		...
		if qtyStr == "0" {
			b.asks.Remove(lookupKey)
		} else {
			b.asks.Remove(lookupKey)
			if entry := b.parseAndConvert(priceStr, qtyStr); entry != nil {
				b.asks.Set(entry, entry)
			}
		}
	}
}
