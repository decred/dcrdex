...
func (dcr *ExchangeWallet) MakeBondTx(amt uint64, lockTime time.Time, acctID []byte) (*asset.Bond, error) {
	...
	txOut := wire.NewTxOut(int64(amt), bondPkScript)
	baseTx.AddTxOut(txOut)
	...
	if err != nil {
		return nil, fmt.Errorf("error creating bond redeem script for address '%v': %w", redeemAddr, err)
	}
	...
	redeemInSig, err := txscript.RawTxInSignature(redeemMsgTx, 0, bondScript, txscript.SigHashAll,
		bondPrivKey, sigType)
	if err != nil {
...
func (dcr *ExchangeWallet) refundBond(txid *chainhash.Hash, vout uint32, amt uint64, script []byte, privKey []byte) ([]byte, error) {
	...
	// CalcSignatureHash and ecsds.Sign with secp256k1 private key.
	redeemInSig, err := txscript.RawTxInSignature(redeemMsgTx, 0, script, txscript.SigHashAll,
