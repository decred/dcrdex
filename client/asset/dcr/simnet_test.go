...
type privateSwapper struct {
	...
	ourRedeemPubKeySecret []byte
	ourRefundPubKey       []byte
	ourRefundPubKeySecret []byte

	cpRedeemPubKey       []byte
	cpRedeemPubKeySecret []byte
	cpRefundPubKey       []byte
	cpRefundPubKeySecret []byte
...
func (s *privateSwapper) generateTakerPreSwapMsg() (*takerPreSwapMsg, error) {
	...
	var err error
	s.ourRedeemPubKey, s.ourRedeemPubKeySecret, err = s.wallet.PrivateSwapPubKey()
	if err != nil {
		return nil, err
	}
	s.ourRefundPubKey, s.ourRefundPubKeySecret, err = s.wallet.PrivateSwapPubKey()
	if err != nil {
		return nil, err
	}
...
func (s *privateSwapper) handleTakerInitSwapMsg(msg *takerInitSwapMsg) (*makerAdaptorsMsg, error) {
	...
	redeemAdaptorSig, err := s.wallet.GeneratePrivateKeyTweakedAdaptor(unsignedRedeem, cpContract, &adaptorSecret, s.ourRedeemPubKeySecret, true)
	if err != nil {
		return nil, fmt.Errorf("error generating redeem adaptor: %w", err)
	}

	refundAdaptorSig, err := s.wallet.GeneratePrivateKeyTweakedAdaptor(s.cpUnsignedRedeem, ourSwapContract, &adaptorSecret, s.ourRefundPubKeySecret, false)
	if err != nil {
...
func (s *privateSwapper) handleMakerAdaptorsMsg(msg *makerAdaptorsMsg) (*takerAdaptorMsg, error) {
	...
	s.ourRefundAdaptorSig, err = s.wallet.GeneratePublicKeyTweakedAdaptor(s.cpUnsignedRedeem, ourSwapContract, s.adaptorPub, s.ourRefundPubKeySecret)
	if err != nil {
...
func (s *privateSwapper) handleTakerAdaptorMsg(msg *takerAdaptorMsg) (*makerRedeemMsg, error) {
	...
	_, _, txData, err := s.wallet.RedeemPrivate(cpSwapContract, s.ourUnsignedRedeem, msg.adaptorSig, s.ourRedeemPubKeySecret, s.adaptorSecret)
	if err != nil {
...
func (s *privateSwapper) handleMakerRedeemMsg(msg *makerRedeemMsg) error {
	...
	adaptorSecret, err := s.wallet.RecoverAdaptorSecret(msg.redeemTx, s.ourRefundAdaptorSig, s.cpRedeemAdaptorSig, s.ourRefundPubKeySecret, s.adaptorPub, ourSwapContract)
	if err != nil {
	...
	_, _, _, err = s.wallet.RedeemPrivate(cpSwapContract, s.ourUnsignedRedeem, s.cpRefundAdaptorSig, s.ourRedeemPubKeySecret, adaptorSecret)
	if err != nil {
