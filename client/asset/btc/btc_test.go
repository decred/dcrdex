...
func tNewWallet() (*ExchangeWallet, *tRPCClient, func()) {
	...
	wallet := newWallet(walletCtx, walletCfg, tLogger, &chaincfg.MainNetParams, client)
...
func TestAvailableFund(t *testing.T) {
	...
	// RPC error
	node.rawErr[methodListUnspent] = tErr
	_, err = wallet.Fund(littleBit - 300)
	if err == nil {
	...
	// Fund a little bit.
	spendables, err := wallet.Fund(littleBit - 300)
	if err != nil {
	...
	// Fund a lotta bit.
	spendables, err = wallet.Fund(lottaBit - 3e4)
	if err != nil {
...
func TestReturnCoins(t *testing.T) {
	...
	// First test it with the local output type.
	coins := asset.Coins{
	...
	err = wallet.ReturnCoins(coins)
	if err == nil {
		t.Fatalf("no error for RPC failure: %v", err)
	}
	node.rawRes[methodLockUnspent] = []byte(`true`)
...
func TestSwap(t *testing.T) {
	...
	_, err := wallet.Swap(swapTx)
	if err != nil {
		t.Fatalf("swap error: %v", err)
	}

	// Not enough funds
	swapTx.Inputs = coins[:1]
	_, err = wallet.Swap(swapTx)
	if err == nil {
