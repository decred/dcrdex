...
type Multisigner interface {
	...
	SendFundsToMultisig(ctx context.Context, pm *PaymentMultisig) (pmTx *PaymentMultisigTx, err error)
	// SignMultisig signs the pmTx with the supplied privateKey and inserts
	// the signature at idx among the other signatures.
	SignMultisig(ctx context.Context, pm *PaymentMultisig, privKey []byte, signIdx int) (pmTx *PaymentMultisigTx, err error)
	// RefundMultisig refunds a multisig if it is after the locktime and we
	// are the sender.
	RefundMultisig(ctx context.Context, pm *PaymentMultisig) error
}
