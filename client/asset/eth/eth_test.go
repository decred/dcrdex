...
var (
	...
	tokenGasesV0 = dexeth.Tokens[simnetTokenID].NetTokens[dex.Simnet].SwapContracts[0].Gas
	ethGasesV1   = dexeth.VersionedGases[1]
	tokenGasesV1 = dexeth.Tokens[simnetTokenID].NetTokens[dex.Simnet].SwapContracts[1].Gas
	...
	tETHV0 = &dex.Asset{
		...
		Symbol:       "ETH",
		MaxFeeRate:   100,
		SwapSize:     ethGasesV0.Swap,
		SwapSizeBase: ethGasesV0.Swap,
		RedeemSize:   ethGasesV0.Redeem,
		SwapConf:     1,
	}
	...
	tETHV1 = &dex.Asset{
		...
		Symbol:       "ETH",
		MaxFeeRate:   100,
		SwapSize:     ethGasesV1.Swap,
		SwapSizeBase: ethGasesV1.Swap,
		RedeemSize:   ethGasesV1.Redeem,
		SwapConf:     1,
	}
	...
	tTokenV0 = &dex.Asset{
		...
		Symbol:       "dextt.eth",
		Version:      0,
		SwapSize:     tokenGasesV0.Swap,
		SwapSizeBase: tokenGasesV0.Swap,
		RedeemSize:   tokenGasesV0.Redeem,
		MaxFeeRate:   20,
		SwapConf:     1,
	}
	...
	tTokenV1 = &dex.Asset{
		...
		Symbol:       "dextt.eth",
		Version:      1,
		SwapSize:     tokenGasesV1.Swap,
		SwapSizeBase: tokenGasesV1.Swap,
		RedeemSize:   tokenGasesV1.Redeem,
		MaxFeeRate:   20,
		SwapConf:     1,
	}
...
func tassetWallet(assetID uint32) (asset.Wallet, *assetWallet, *tMempoolNode, context.CancelFunc) {
	...
	aw := &assetWallet{
		...
		assetID:            assetID,
		contractors:        map[uint32]contractor{0: c, 1: c},
	...
	if assetID == BipID {
		...
		node.tokenParent = &assetWallet{
			...
			log:              tLogger.SubLogger("ETH"),
			contractors:      map[uint32]contractor{0: node.tContractor, 1: node.tContractor},
			assetID:          BipID,
			atomize:          dexeth.WeiToGwei,
			pendingApprovals: make(map[uint32]*pendingApproval),
			approvalCache:    make(map[uint32]bool),
...
func testFundOrderReturnCoinsFundingCoins(t *testing.T, assetID uint32) {
	...
	eth2.contractors[1] = node.tokenContractor
	node.tokenContractor.bal = dexeth.GweiToWei(walletBalanceGwei)
...
func testRedeem(t *testing.T, assetID uint32) {
	...
	if assetID == BipID {
		contractor = eth.contractors[0].(*tContractor)
	} else {
		contractor = eth.contractors[0].(*tTokenContractor).tContractor
	}
	...
	*/
	...
	for _, test := range tests {
		...
		txHash := contractor.redeemTx.Hash()
...
func testMaxSwapRedeemLots(t *testing.T, assetID uint32) {
	...
	if assetID == BipID {
		...
		if info.MaxSwapsInTx != 22 {
			t.Fatalf("expected 22 for max swaps but got %d", info.MaxSwapsInTx)
		}
		if info.MaxRedeemsInTx != 60 {
			t.Fatalf("expected 60 for max redemptions but got %d", info.MaxRedeemsInTx)
		}
	}
}
