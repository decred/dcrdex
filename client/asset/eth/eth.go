...
func (w *ETHWallet) AlreadyInitialized(assetVersion uint32, contract *asset.Contract) (initialized bool, zeroHash, contractData []byte, err error) {
	contractVer := contractVersion(assetVersion)
	locator := acToLocator(contractVer, contract, dexeth.GweiToWei(contract.Value), w.addr)
...
// AlreadyInitialized returns if the swap was already initialized and some data
// needed to interact with the contract later.
func (w *TokenWallet) AlreadyInitialized(assetVersion uint32, contract *asset.Contract) (initialized bool, zeroHash, contractData []byte, err error) {
	contractVer := contractVersion(assetVersion)
