...
const (
	...
	contractVersionUnknown = contractVersionNewest - 1
)
...
type baseWallet struct {
	...
	monitoredTxsMtx sync.RWMutex //nolint:structcheck
...
func (w *assetWallet) estimateSwap(lots, lotSize, feeSuggestion uint64, dexSwapCfg, dexRedeemCfg *dex.Asset) (*asset.SwapEstimate, error) {
	...
	oneSwap, nSwap, _, err := w.swapGas(int(lots), dexSwapCfg)
	if err != nil {
...
func (w *assetWallet) gases(serverVer uint32) *dexeth.Gases {
	return gases(w.assetID, contractVersion(serverVer), w.net)
}
...
func (w *TokenWallet) FundOrder(ord *asset.Order) (asset.Coins, []dex.Bytes, error) {
	...
	if err := w.maybeApproveTokenSwapContract(ord.DEXConfig, ethToLock); err != nil {
...
func (w *assetWallet) swapGas(n int, dexCfg *dex.Asset) (oneSwap, nSwap uint64, approved bool, err error) {
	oneSwap = dexCfg.SwapSize

	g := w.gases(dexCfg.Version)
	if g == nil {
		return 0, 0, false, fmt.Errorf("no gases known for %d version %d", w.assetID, dexCfg.Version)
	}
...
func (w *assetWallet) redeemGas(n int, nfo *dex.Asset) (oneGas, nGas uint64, err error) {
	...
	if g.Redeem > redeemGas {
		...
		w.log.Debugf("Server's reported redeem gas estimate %d is less than ours %d. Using our estimate.", redeemGas, g.Redeem)
		redeemGas = g.Redeem
...
func (r *swapReceipt) Contract() dex.Bytes {
	return dexeth.EncodeContractData(r.serverVer, r.secretHash)
}
...
func (w *ETHWallet) Swap(swaps *asset.Swaps) ([]asset.Receipt, asset.Coin, uint64, error) {
	...
	oneSwap, _, _, err := w.swapGas(1, cfg)
	if err != nil {
...
func (w *TokenWallet) Swap(swaps *asset.Swaps) ([]asset.Receipt, asset.Coin, uint64, error) {
	...
	oneSwap, _, approved, err := w.swapGas(1, cfg)
	if err != nil {
		return fail("error getting gas fees: %v", err)
	}
...
func (w *assetWallet) Redeem(form *asset.RedeemForm, feeWallet *assetWallet, nonceOverride *uint64) ([]dex.Bytes, asset.Coin, uint64, error) {
	...
	var serverVer uint32 // require a consistent version since this is a single transaction
	secrets := make([][32]byte, 0, n)
	var redeemedValue uint64
	details := make([]*dex.SwapContractDetails, len(form.Redemptions))
	for i, redemption := range form.Redemptions {
		...
		if i == 0 {
			serverVer = ver
		} else if serverVer != ver {
			return fail(fmt.Errorf("Redeem: inconsistent server versions in RedeemForm.Redemptions: "+
				"%d != %d", serverVer, ver))
		}
		...
		deets := redemption.SwapDetails
		details[i] = deets
		redeemable, err := w.isRedeemable(secret, deets, ver)
		if err != nil {
		...
		step, _, _, err := w.status(w.ctx, deets, serverVer)
		if err != nil {
			return nil, nil, 0, fmt.Errorf("error finding swap state: %w", err)
		}

		if step != dexeth.SSInitiated {
			return nil, nil, 0, fmt.Errorf("cannot redeem swap in state %s", step)
		}

		redeemedValue += deets.Value
	}
	...
	gasEst, err := w.estimateRedeemGas(w.ctx, secrets, details, serverVer)
	if err != nil {
	...
	tx, err := w.redeem(w.ctx, w.assetID, form.Redemptions, gasFeeCap, gasLimit, serverVer, nonceOverride)
	if err != nil {
...
func (w *TokenWallet) maybeApproveTokenSwapContract(dexCfg *dex.Asset, swapReserves uint64) error {
	...
	approveGas, err := w.approvalGas(unlimitedAllowance, dexCfg)
	if err != nil {
		return fmt.Errorf("error estimating allowance gas: %w", err)
	}
	if (approveGas*dexCfg.MaxFeeRate)+swapReserves > ethBal.Available {
		return fmt.Errorf("parent balance %d doesn't cover contract approval (%d) and tx fees (%d)",
			ethBal.Available, approveGas*dexCfg.MaxFeeRate, swapReserves)
	}
	tx, err := w.approveToken(unlimitedAllowance, dexCfg.MaxFeeRate, dexCfg.Version)
	if err != nil {
		return fmt.Errorf("token contract approval error (using max fee rate %d): %w", dexCfg.MaxFeeRate, err)
	}
...
func (w *assetWallet) approveToken(amount *big.Int, maxFeeRate uint64, serverVer uint32) (tx *types.Transaction, err error) {
	w.nonceSendMtx.Lock()
	...
	return tx, w.withTokenContractor(w.assetID, serverVer, func(c tokenContractor) error {
...
func (w *TokenWallet) ReserveNRedemptions(n uint64, dexRedeemCfg *dex.Asset) (uint64, error) {
	...
	reserve := g.Redeem * dexRedeemCfg.MaxFeeRate * n
...
func (w *assetWallet) AuditContract(coinID, contract, serializedTx dex.Bytes, rebroadcast bool) (*asset.AuditInfo, error) {
	...
	case 1:
		details, err := dexeth.ParseInitiateDataV1(tx.Data())
		if err != nil {
			return nil, fmt.Errorf("AuditContract: failed to parse initiate data: %w", err)
		}

		deets, ok := details[secretHash]
		if !ok {
			return nil, errors.New("AuditContract: tx does not initiate secret hash")
		}
		val = deets.Value
		participant = deets.To
		initiator = deets.From
		lockTime = time.Unix(int64(deets.LockTime), 0)
	default:
...
func (w *assetWallet) LocktimeExpired(ctx context.Context, deets *dex.SwapContractDetails) (bool, time.Time, error) {
	header, err := w.node.bestHeader(ctx)
	if err != nil {
		return false, time.Time{}, err
	}

	return deets.LockTime < header.Time, time.Unix(int64(deets.LockTime), 0), nil
}
...
// is seen.
func (w *assetWallet) FindRedemption(ctx context.Context, _, contractData dex.Bytes, deets *dex.SwapContractDetails) (redemptionCoin, secret dex.Bytes, err error) {
	serverVer, secretHash, err := dexeth.DecodeContractData(contractData)
	if err != nil {
		return nil, nil, err
	}

	// See if it's ready right away.
	secret, err = w.findSecret(deets, serverVer)
	if err != nil {
		return nil, nil, err
	}
...
func (w *assetWallet) findSecret(deets *dex.SwapContractDetails, serverVer uint32) ([]byte, error) {
	ctx, cancel := context.WithTimeout(w.ctx, 10*time.Second)
	defer cancel()
	step, secret, _, err := w.status(ctx, deets, serverVer)
	if err != nil {
		return nil, err
	}
	...
	case dexeth.SSRedeemed:
		return secret[:], nil
	case dexeth.SSNone:
		return nil, fmt.Errorf("swap %x does not exist", deets.SecretHash.String())
	case dexeth.SSRefunded:
		return nil, fmt.Errorf("swap %x is already refunded", deets.SecretHash.String())
	}
	return nil, fmt.Errorf("unrecognized swap state %v", step)
}

// Refund refunds a contract. This can only be used after the time lock has
// expired.
func (w *assetWallet) Refund(coinID, contractData dex.Bytes, deets *dex.SwapContractDetails, feeRate uint64) (dex.Bytes, error) {
	version, secretHash, err := dexeth.DecodeContractData(contractData)
	if err != nil {
		return nil, fmt.Errorf("Refund: failed to decode contract: %w", err)
	}

	step, secret, _, err := w.status(w.ctx, deets, version)
	if err != nil {
		return nil, err
	}
	...
	refundable, err := w.isRefundable(deets, version)
	if err != nil {
	...
	tx, err := w.refund(deets, feeRate, version)
	if err != nil {
...
func (w *assetWallet) SwapConfirmations(ctx context.Context, coinID dex.Bytes, contractData dex.Bytes, _ time.Time, deets *dex.SwapContractDetails) (confs uint32, spent bool, err error) {
	...
	step, _, blockNumber, err := w.status(w.ctx, deets, contractVer)
	if err != nil {
	...
	spent = step >= dexeth.SSRedeemed
	if !spent {
		confs = uint32(hdr.Number.Uint64()) - blockNumber + 1
	}
...
func (w *assetWallet) resubmitRedemption(tx *types.Transaction, redeem *asset.Redemption, serverVer uint32, nonceOverride *uint64,
	feeWallet *assetWallet, monitoredTx *monitoredTx) (*common.Hash, error) {

	makeRedemption := func(deets *dex.SwapContractDetails, secret [32]byte) *asset.Redemption {
		var secretHash [dexeth.SecretHashSize]byte
		copy(secretHash[:], deets.SecretHash)
		contractData := dexeth.EncodeContractData(serverVer, secretHash)
		return &asset.Redemption{
			Spends: &asset.AuditInfo{
				Contract:   contractData,
				SecretHash: deets.SecretHash[:],
			},
			SwapDetails: deets,
			Secret:      secret[:],
		}
	...
	var redemptions []*asset.Redemption
	switch serverVer {
	case 0:
		parsedRedemptions, err := dexeth.ParseRedeemDataV0(tx.Data())
		if err != nil {
			return nil, fmt.Errorf("failed to parse redeem data: %w", err)
		}
		redemptions = make([]*asset.Redemption, 0, len(parsedRedemptions))
		if err := w.withContractor(serverVer, func(ci contractor) error {
			c := ci.(interface {
				swap(context.Context, [32]byte) (*dexeth.SwapState, error)
			})
			// Whether or not a a swap can be redeemed is checked in Redeem, but here
			...
			for _, r := range parsedRedemptions {
				swap, err := c.swap(context.TODO(), r.SecretHash)
				if err != nil {
					return err
				} else if swap.State != dexeth.SSInitiated {
					w.log.Warnf("swap %x is not redeemable. not resubmitting", r.SecretHash)
					continue
				}

				redemptions = append(redemptions, makeRedemption(&dex.SwapContractDetails{
					From:       swap.Initiator.String(),
					To:         swap.Participant.String(),
	...
	case 1:
		...
		for _, r := range redeems {
			is, err := w.isRedeemable(r.Secret, r.Contract, serverVer)
			if err != nil {
				return nil, err
			} else if !is {
				w.log.Warnf("swap %x is not redeemable. not resubmitting", r.Contract.SecretHash)
				continue
			}
			redemptions = append(redemptions, makeRedemption(r.Contract, r.Secret))
		}
	}
...
func (w *assetWallet) swapIsRedeemed(deets *dex.SwapContractDetails, serverVer uint32) (bool, error) {
	step, _, _, err := w.status(w.ctx, deets, serverVer)
	if err != nil {
		return false, err
	}
...
func (w *assetWallet) checkUnconfirmedRedemption(redeem *asset.Redemption, contractVer uint32, txHash common.Hash, tx *types.Transaction, currentTip uint64, feeWallet *assetWallet, monitoredTx *monitoredTx) (*asset.ConfirmRedemptionStatus, error) {
	// Check if the swap has been redeemed by another transaction we are unaware of.
	swapIsRedeemed, err := w.swapIsRedeemed(redeem.SwapDetails, contractVer)
	if err != nil {
		return nil, err
	}
	if swapIsRedeemed {
	...
	if confirmedNonce > tx.Nonce() {
		replacementTxHash, err := w.resubmitRedemption(tx, redeem, contractVer, nil, feeWallet, monitoredTx)
		if err != nil {
			return nil, err
		}
	...
	if baseFee.Cmp(tx.GasFeeCap()) > 0 {
		...
		nonce := tx.Nonce()
		replacementTxHash, err := w.resubmitRedemption(tx, redeem, contractVer, &nonce, feeWallet, monitoredTx)
		if err != nil {
			return nil, err
		}
		return confStatus(0, *replacementTxHash), nil
...
func (w *assetWallet) confirmRedemptionWithoutMonitoredTx(txHash common.Hash, redeem *asset.Redemption, feeWallet *assetWallet) (*asset.ConfirmRedemptionStatus, error) {
	serverVer, _, err := dexeth.DecodeContractData(redeem.Spends.Contract)
	if err != nil {
	...
	if errors.Is(err, asset.CoinNotFoundError) {
		...
		swapIsRedeemed, err := w.swapIsRedeemed(redeem.SwapDetails, serverVer)
		if err != nil {
			return nil, err
		}
		if swapIsRedeemed {
		...
		txs, _, _, err := w.Redeem(&asset.RedeemForm{
			Redemptions:   []*asset.Redemption{redeem},
	...
	if confirmations >= txConfsNeededToConfirm {
		...
		replacementTxHash, err := w.resubmitRedemption(tx, redeem, serverVer, nil, feeWallet, nil)
		if err != nil {
			return nil, err
		}
		return confStatus(0, *replacementTxHash), nil
	...
	return w.checkUnconfirmedRedemption(redeem, serverVer, txHash, tx, currentTip, feeWallet, nil)
}
...
// cannot be mined.
func (w *assetWallet) confirmRedemption(coinID dex.Bytes, redeem *asset.Redemption, feeWallet *assetWallet) (*asset.ConfirmRedemptionStatus, error) {
	...
	if err != nil {
		w.log.Error(err)
		return w.confirmRedemptionWithoutMonitoredTx(txHash, redeem, feeWallet)
	}
	...
	contractVer, _, err := dexeth.DecodeContractData(redeem.Spends.Contract)
	if err != nil {
		return nil, fmt.Errorf("failed to decode contract data: %w", err)
	}
	hdr, err := w.node.bestHeader(w.ctx)
	...
	if errors.Is(err, asset.CoinNotFoundError) {
		...
		replacementTxHash, err := w.resubmitRedemption(monitoredTx.tx, redeem, contractVer, nil, feeWallet, monitoredTx)
		if err != nil {
			return nil, err
		}
		return confStatus(0, *replacementTxHash), nil
	...
	return w.checkUnconfirmedRedemption(redeem, contractVer, txHash, tx, currentTip, feeWallet, monitoredTx)
}
...
func (w *assetWallet) checkFindRedemptions() {
	for secretHash, req := range w.findRedemptionRequests() {
		secret, err := w.findSecret(req.deets, req.serverVer)
		if err != nil {
...
// status returns the current status of the swap. For v1 swaps, whether secret
// or blockNumber or neither is set depends on the current step. For
// SSNone and SSRefunded, neither is set. For SSInitiated, the blockNumber is
// set. For SSRedeemed, the secret is set.
func (w *assetWallet) status(ctx context.Context, deets *dex.SwapContractDetails, serverVer uint32) (step dexeth.SwapStep, secret [32]byte, blockNumber uint32, err error) {
	return step, secret, blockNumber, w.withContractor(serverVer, func(c contractor) error {
		step, secret, blockNumber, err = c.status(ctx, deets)
		if err != nil {
			return err
		}
		return nil
...
func (w *assetWallet) initiate(ctx context.Context, assetID uint32, details []*dex.SwapContractDetails,
	maxFeeRate, gasLimit uint64, serverVer uint32) (tx *types.Transaction, err error) {
	...
	return tx, w.withContractor(serverVer, func(c contractor) error {
...
func (w *assetWallet) estimateRedeemGas(ctx context.Context, secrets [][32]byte, details []*dex.SwapContractDetails, serverVer uint32) (gas uint64, err error) {
	return gas, w.withContractor(serverVer, func(c contractor) error {
		gas, err = c.estimateRedeemGas(ctx, secrets, details)
		return err
	})
}

// estimateRefundGas checks the amount of gas that is used for a refund.
func (w *assetWallet) estimateRefundGas(ctx context.Context, details *dex.SwapContractDetails, contractVer uint32) (gas uint64, err error) {
	return gas, w.withContractor(contractVer, func(c contractor) error {
		gas, err = c.estimateRefundGas(ctx, details)
		return err
	})
}
...
func (w *assetWallet) withContractor(serverVer uint32, f func(contractor) error) error {
...
func (w *assetWallet) withTokenContractor(assetID, serverVer uint32, f func(tokenContractor) error) error {
	return w.withContractor(serverVer, func(c contractor) error {
...
func (w *assetWallet) redeem(ctx context.Context, assetID uint32, redemptions []*asset.Redemption,
	maxFeeRate, gasLimit uint64, serverVer uint32, nonceOverride *uint64) (tx *types.Transaction, err error) {
...
// this to error.
func (w *assetWallet) refund(deets *dex.SwapContractDetails, maxFeeRate uint64, serverVer uint32) (tx *types.Transaction, err error) {
	gas := w.gases(serverVer)
	if gas == nil {
		return nil, fmt.Errorf("no gas table for server %d, version %d", w.assetID, serverVer)
	}
	w.nonceSendMtx.Lock()
	defer w.nonceSendMtx.Unlock()
	...
	return tx, w.withContractor(serverVer, func(c contractor) error {
		tx, err = c.refund(txOpts, deets)
		return err
	})
}

// isRedeemable checks if the swap identified by secretHash is redeemable using secret.
func (w *assetWallet) isRedeemable(secret [32]byte, deets *dex.SwapContractDetails, serverVer uint32) (redeemable bool, err error) {
	return redeemable, w.withContractor(serverVer, func(c contractor) error {
		redeemable, err = c.isRedeemable(secret, deets)
		return err
	})
}

func (w *assetWallet) isRefundable(deets *dex.SwapContractDetails, serverVer uint32) (refundable bool, err error) {
	return refundable, w.withContractor(serverVer, func(c contractor) error {
		refundable, err = c.isRefundable(deets)
		return err
	})
}
...
func GetGasEstimates(ctx context.Context, cl *nodeClient, c contractor, maxSwaps int, g *dexeth.Gases, waitForMined func()) error {
	...
	for n := 1; n <= maxSwaps; n++ {
		...
		details := make([]*dex.SwapContractDetails, 0, n)
		secrets := make([][32]byte, 0, n)
		...
		for i := 0; i < n; i++ {
			...
			deets := &dex.SwapContractDetails{
				To:         cl.address().String(),
				From:       cl.address().String(),
				Value:      1,
				SecretHash: secretHash[:],
				LockTime:   uint64(time.Now().Add(-time.Hour).Unix()),
			}
			details = append(details, deets)
			secrets = append(secrets, secret)
		...
		refundGas, err := c.estimateRefundGas(ctx, details[0])
		if err != nil {
			return fmt.Errorf("error estimating refund gas: %w", err)
		}
