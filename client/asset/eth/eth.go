...
func (w *TokenWallet) FundOrder(ord *asset.Order) (asset.Coins, []dex.Bytes, uint64, error) {
	...
	approvalStatus, err := w.tokenSwapContractApprovalStatus(ord.Version)
	if err != nil {
...
func (w *TokenWallet) ApprovalStatus() map[uint32]asset.ApprovalStatus {
	...
	for _, version := range versions {
		status, err := w.tokenSwapContractApprovalStatus(version)
		if err != nil {
...
func (w *BridgeWallet) ApproveBridgeContract(ctx context.Context) (string, error) {
	...
	feeRateGwei := dexeth.WeiToGweiCeil(maxFeeRate)
	approvalGas, err := w.approvalGas(unlimitedAllowance, contractVersionNewest)
	if err != nil {
		return "", fmt.Errorf("error calculating approval gas: %w", err)
	}
	...
	var txID string
	return txID, w.withNonce(ctx, func(nonce *big.Int) (*types.Transaction, asset.TransactionType, uint64, error) {
		txOpts, err := w.node.txOpts(ctx, 0, approvalGas, maxFeeRate, tipRate, nonce)
		if err != nil {
			return nil, 0, 0, fmt.Errorf("txOpts error: %w", err)
		}

		tx, err := w.bridge.approveBridgeContract(txOpts, unlimitedAllowance)
		if err != nil {
			return nil, 0, 0, fmt.Errorf("error approving bridge contract: %w", err)
		}
		...
		return tx, asset.ApproveToken, w.atomize(unlimitedAllowance), nil
	})
}
...
func (w *BridgeWallet) UnapproveBridgeContract(ctx context.Context) (string, error) {
	...
	feeRateGwei := dexeth.WeiToGweiCeil(maxFeeRate)
	approvalGas, err := w.approvalGas(new(big.Int), contractVersionNewest)
	if err != nil {
		return "", fmt.Errorf("error calculating approval gas: %w", err)
	}
	...
	return txID, w.withNonce(ctx, func(nonce *big.Int) (*types.Transaction, asset.TransactionType, uint64, error) {
		...
		return tx, asset.ApproveToken, w.atomize(new(big.Int)), nil
	})
}
...
func (w *BridgeWallet) Bridge(ctx context.Context, amt uint64, dest uint32) (txID string, err error) {
	...
	return txID, w.withNonce(ctx, func(nonce *big.Int) (*types.Transaction, asset.TransactionType, uint64, error) {
		...
		txID = tx.Hash().Hex()
		return tx, asset.BridgeToken, amt, nil
	})
}
...
func (w *BridgeWallet) Mint(ctx context.Context, mintData []byte) (txID string, err error) {
	...
	return txID, w.withNonce(ctx, func(nonce *big.Int) (*types.Transaction, asset.TransactionType, uint64, error) {
		...
		return tx, asset.MintToken, 0, nil
	})
}
