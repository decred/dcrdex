...
func (db *TxDB) v1Upgrade(tx *badger.Txn) error {
	err := db.ReIndex(tx, txsTable, allAssetIndexName, func(k, v []byte) ([]byte, error) {
	...
	return db.ReIndex(tx, txsTable, assetIndexName, func(k, v []byte) ([]byte, error) {
		wt := new(extendedWalletTx)
		if err := wt.UnmarshalBinary(v); err != nil {
			return nil, err
		}
...
func (db *TxDB) upgrade() error {
	...
	return db.Update(func(tx *badger.Txn) error {
		...
		return db.SetDBVersion(dbVersion, tx)
	})
}
