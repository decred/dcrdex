...
func (w *ExchangeWallet) Connect(ctx context.Context) (*sync.WaitGroup, error) {
	...
	switch connStatus {
	case cgo.ConnectionDisconnected:
		return nil, errors.New("failed to connect to daemon: disconnected")
	case cgo.ConnectionWrongVersion:
...
func (w *ExchangeWallet) checkPeers() {
	...
	var numPeers uint32
	if wallet.Connected() == cgo.ConnectionConnected {
		numPeers = 1
	}
...
func (w *ExchangeWallet) commitTx(tx *cgo.PendingTransaction, sendAmount uint64) (*Coin, error) {
