...
/*
Package cgo provides Go bindings to monero_c, a C wrapper around Monero's
wallet2 library.
...
# Usage
	...
	wallet, err := wm.CreateWallet("/path/to/wallet", "password", "English", cgo.NetworkMainnet)
	if err != nil {
		log.Fatal(err)
	}
	...
	// Send transaction
	tx, err := wallet.CreateTransaction(destAddr, amount, cgo.PriorityDefault, 0)
	if err != nil {
		log.Fatal(err)
	}
