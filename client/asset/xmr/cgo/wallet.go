...
// Package cgo provides Go bindings to monero_c, a C wrapper around
...
// Network types
const (
	NetworkMainnet  = C.NetworkType_MAINNET
	NetworkTestnet  = C.NetworkType_TESTNET
	NetworkStagenet = C.NetworkType_STAGENET
)

// Transaction priority levels
const (
	PriorityDefault = C.Priority_Default
	PriorityLow     = C.Priority_Low
	PriorityMedium  = C.Priority_Medium
	PriorityHigh    = C.Priority_High
	PriorityLast    = C.Priority_Last
)

// Wallet status codes
const (
	StatusOk       = C.WalletStatus_Ok
	StatusError    = C.WalletStatus_Error
	StatusCritical = C.WalletStatus_Critical
)

// Connection status codes
const (
	ConnectionDisconnected = C.WalletConnectionStatus_Disconnected
	ConnectionConnected    = C.WalletConnectionStatus_Connected
	ConnectionWrongVersion = C.WalletConnectionStatus_WrongVersion
)
...
func (wm *WalletManager) CreateWallet(path, password, language string, networkType int) (*Wallet, error) {
...
func (wm *WalletManager) OpenWallet(path, password string, networkType int) (*Wallet, error) {
	cPath := C.CString(path)
	cPassword := C.CString(password)
	defer C.free(unsafe.Pointer(cPath))
	defer C.free(unsafe.Pointer(cPassword))
...
func (wm *WalletManager) RecoveryWallet(path, password, mnemonic string, networkType int, restoreHeight uint64) (*Wallet, error) {
	cPath := C.CString(path)
	cPassword := C.CString(password)
...
func (wm *WalletManager) CreateWalletFromKeys(path, password, language string, networkType int, restoreHeight uint64, address, viewKey, spendKey string) (*Wallet, error) {
	cPath := C.CString(path)
	cPassword := C.CString(password)
	cLanguage := C.CString(language)
...
func (wm *WalletManager) CreateDeterministicWalletFromSpendKey(path, password, language string, networkType int, restoreHeight uint64, spendKey string) (*Wallet, error) {
	cPath := C.CString(path)
	cPassword := C.CString(password)
	cLanguage := C.CString(language)
	cSpendKey := C.CString(spendKey)
...
func (w *Wallet) Status() int {
	return int(C.MONERO_Wallet_status(w.ptr))
}
...
func (w *Wallet) Connected() int {
	return int(C.MONERO_Wallet_connected(w.ptr))
}
...
func AddressValid(address string, networkType int) bool {
	cAddr := C.CString(address)
	defer C.free(unsafe.Pointer(cAddr))
...
func (w *Wallet) CreateTransaction(destAddr string, amount uint64, priority int, accountIndex uint32) (*PendingTransaction, error) {
...
func (w *Wallet) SweepAll(destAddr string, priority int, accountIndex uint32) (*PendingTransaction, error) {
	cDest := C.CString(destAddr)
	cDestSep := C.CString(",")
	cPaymentID := C.CString("")
	cAmounts := C.CString("")
...
func (tx *PendingTransaction) Status() int {
	return int(C.MONERO_PendingTransaction_status(tx.ptr))
}
...
func (ti *TransactionInfo) Direction() int {
	return int(C.MONERO_TransactionInfo_direction(ti.ptr))
}
