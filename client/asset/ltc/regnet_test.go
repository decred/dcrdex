...
const (
	gammaAddress  = "moWi3tkhqaRVwpEQGqRyipRLknJ67LTocu"
	gammaPassword = "abc"
	alphaAddress  = "mt9hgfXXbM3x7hewgEAovBwqoMAAnctJ4V"
	betaAddress   = "mzWm7aRjnZZCAiEkidzbthtVBzNXfNkA4e"
)
...
func mineAlpha() error {
	return exec.Command("tmux", "send-keys", "-t", "ltc-harness:2", "./mine-alpha 1", "C-m").Run()
}

func tBackend(t *testing.T, conf, name string, blkFunc func(string, error)) (*btc.ExchangeWallet, *dex.ConnectionMaster) {
	user, err := user.Current()
	if err != nil {
		t.Fatalf("error getting current user: %v", err)
	}
	cfgPath := filepath.Join(user.HomeDir, "dextest", "ltc", "harness-ctl", conf+".conf")
...
func TestWallet(t *testing.T) {
	...
	for name, wallet := range rig.backends {
		...
		if len(available) == 0 {
			t.Fatalf("no amount available for wallet %q", name)
		}
		a := available[0]
		tLogger.Debugf("%q %d available, %d immature", name, a.Available/1e8, a.Immature/1e8)
	}
