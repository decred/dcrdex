...
func (db *BoltDB) StoreMultisigIndexForPubkey(assetID, idx uint32, pubkey [32]byte) error {
	return db.Update(func(tx *bbolt.Tx) error {
		outerBkt := tx.Bucket(multisigIndexesBucket)
		if outerBkt == nil {
			return errors.New("no multisig indexes bucket")
		}
...
func (db *BoltDB) MultisigIndexForPubkey(assetID uint32, pubkey [32]byte) (uint32, error) {
	var multisigIndex uint32
...
//StoreMultisigIndexForPubkey(assetID uint32, pubkey [33]byte, idx uint32) error
