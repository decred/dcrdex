...
type Report struct {
	...
	SpreadPct     float64       `json:"avgSpreadPct,omitempty"`
...
func verify(snapshotsData []byte, pubkeyHex string, startEpoch, endEpoch, minQty uint64, maxSpreadPct float64) (*Report, error) {
	...
	if spreadCount > 0 {
		report.SpreadPct = math.Round(spreadSum/float64(spreadCount)*100) / 100
	}
