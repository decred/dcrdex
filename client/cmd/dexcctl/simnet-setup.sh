...
if [ $ZEC_ON -eq 0 ]; then
	...
	./dexcctl -p abc -p "" --simnet newwallet 133 zcashdRPC ~/dextest/zec/beta/beta.conf
fi
...
if [ $ZCL_ON -eq 0 ]; then
	...
	./dexcctl -p abc -p "" --simnet newwallet 147 zclassicdRPC ~/dextest/zcl/alpha/alpha.conf
fi
