...
export class NetworkAsset {
  ...
  constructor (a: SupportedAsset) {
    ...
    this.symbol = symbol
    this.ui = ui
    this.chainName = token ? app.user.assets[token.parentID].name : name
    this.chainLogo = token ? Doc.logoPath(app.user.assets[token.parentID].symbol) : Doc.logoPath(symbol)
    if (token) this.token = { parentID: token.parentID, feeUI: app.unitInfo(token.parentID), parentMade: Boolean(app.user.assets[token.parentID].wallet) }
  }

  get bal () {
    const w = app.user.assets[this.assetID].wallet
    return w?.balance ?? { available: 0, locked: 0, immature: 0 }
  }

  updateTokenParentMade () {
    if (!this.token) return false
    this.token.parentMade = Boolean(app.user.assets[this.token.parentID].wallet)
  }
}
...
export class TickerAsset {
  ...
  addNetworkAsset (a: SupportedAsset) {
    ...
    const xcRate = app.idRate(assetID)
  ...
  updateHasWallets () {
    for (const ta of this.networkAssets) {
      ...
      if (app.wallet(assetID) || token?.parentMade) {
  ...
  blockchainWallet () {
    for (const { assetID } of this.networkAssets) {
      const { wallet, token } = app.asset(assetID)
