...
function perLotRequirements (
  ...
  const calculateTotalAmount = (perLot: PerLotBreakdown) : number => {
    ...
    total *= (1 + (perLot.slippageBuffer / 100) + (perLot.multiSplitBuffer / 100))
...
function toAllocateRunning (
  ...
  for (const assetID of assetIDs) {
    ...
    if (dexSurplus >= 0) {
      ...
      if (result.dex[assetID].amount < 0) result.dex[assetID].amount = Math.max(result.dex[assetID].amount, result.dex[assetID].calculation.runningBotAvailable)
    } else {
      result.dex[assetID].status = 'insufficient'
      result.dex[assetID].amount = result.dex[assetID].calculation.available
    }
    ...
    if (cexSurplus >= 0) {
      ...
      if (result.cex[assetID].amount < 0) result.cex[assetID].amount = Math.max(result.cex[assetID].amount, result.cex[assetID].calculation.runningBotAvailable)
    } else {
      result.cex[assetID].status = 'insufficient'
      result.cex[assetID].amount = result.cex[assetID].calculation.available
    }
