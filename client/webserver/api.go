...
func (s *WebServer) apiNewWallet(w http.ResponseWriter, r *http.Request) {
	...
	if has {
		s.writeAPIError(w, core.NewError("already have a wallet for %s", unbip(form.AssetID)))
		return
	}
...
func (s *WebServer) apiOpenWallet(w http.ResponseWriter, r *http.Request) {
	...
	if status == nil {
		s.writeAPIError(w, core.NewError("No wallet for %d -> %s", form.AssetID, unbip(form.AssetID)))
		return
	}
	err := s.core.OpenWallet(form.AssetID, form.Pass)
	if err != nil {
		s.writeAPIError(w, core.NewError("error unlocking %s wallet: %v", unbip(form.AssetID), err))
		return
	}
...
func (s *WebServer) apiNewDepositAddress(w http.ResponseWriter, r *http.Request) {
	...
	if form.AssetID == nil {
		s.writeAPIError(w, core.NewError("missing asset ID"))
		return
	}
	assetID := *form.AssetID
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error connecting to %s wallet: %v", unbip(assetID), err))
		return
	}
...
func (s *WebServer) apiConnectWallet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error connecting to %s wallet: %v", unbip(form.AssetID), err))
		return
	}
...
func (s *WebServer) apiTrade(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error placing order: %v", err))
		return
	}
	resp := &struct {
		OK    bool        `json:"ok"`
...
func (s *WebServer) apiAccountExport(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error exporting account: %v", err))
		return
	}
	w.Header().Set("Connection", "close")
	res := &struct {
...
func (s *WebServer) apiAccountImport(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error importing account: %v", err))
		return
	}
	w.Header().Set("Connection", "close")
	writeJSON(w, simpleAck(), s.indent)
...
func (s *WebServer) apiAccountDisable(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error disabling account: %v", err))
		return
	}
	w.Header().Set("Connection", "close")
	writeJSON(w, simpleAck(), s.indent)
...
func (s *WebServer) apiCancel(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error cancelling order %s: %v", form.OrderID, err))
		return
	}
	writeJSON(w, simpleAck(), s.indent)
}
...
func (s *WebServer) apiCloseWallet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error locking %s wallet: %v", unbip(form.AssetID), err))
		return
	}
...
func (s *WebServer) apiInit(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("initialization error: %v", err))
		return
	}
...
func (s *WebServer) apiIsInitialized(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("isinitialized error: %v", err))
		return
	}
	writeJSON(w, &struct {
		OK          bool `json:"ok"`
...
func (s *WebServer) apiLogout(w http.ResponseWriter, r *http.Request) {
	err := s.core.Logout()
	if err != nil {
		s.writeAPIError(w, core.NewError("logout error: %v", err))
		return
	}
...
func (s *WebServer) apiGetBalance(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("balance error: %v", err))
		return
	}
	resp := &struct {
		OK      bool                `json:"ok"`
...
func (s *WebServer) apiParseConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("parse error: %v", err))
		return
	}
	resp := &struct {
		OK  bool              `json:"ok"`
...
func (s *WebServer) apiWalletSettings(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error setting wallet settings: %v", err))
		return
	}
	writeJSON(w, &struct {
		OK  bool              `json:"ok"`
...
func (s *WebServer) apiDefaultWalletCfg(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("error getting wallet config: %v", err))
		return
	}
	writeJSON(w, struct {
		OK     bool              `json:"ok"`
...
func (s *WebServer) apiOrders(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("Orders error: %v", err))
		return
	}
	writeJSON(w, &struct {
		OK     bool          `json:"ok"`
...
func (s *WebServer) apiOrder(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("Order error: %v", err))
		return
	}
	writeJSON(w, &struct {
		OK    bool        `json:"ok"`
...
func (s *WebServer) apiChangeAppPass(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("change app pass error: %v", err))
		return
	}
...
func (s *WebServer) apiReconfig(w http.ResponseWriter, r *http.Request) {
	...
	err := s.core.ReconfigureWallet(form.AppPW, form.NewWalletPW, form.AssetID,
		form.Config)
	if err != nil {
		s.writeAPIError(w, core.NewError("reconfig error: %v", err))
		return
	}
...
func (s *WebServer) apiWithdraw(w http.ResponseWriter, r *http.Request) {
	...
	if state == nil {
		s.writeAPIError(w, core.NewError("no wallet found for %s", unbip(form.AssetID)))
		return
	}
	coin, err := s.core.Withdraw(form.Pass, form.AssetID, form.Value, form.Address)
	if err != nil {
		s.writeAPIError(w, core.NewError("withdraw error: %v", err))
		return
	}
	resp := struct {
		OK   bool   `json:"ok"`
...
func (s *WebServer) apiMaxBuy(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("max order estimation error: %v", err))
		return
	}
	resp := struct {
		OK     bool                   `json:"ok"`
...
func (s *WebServer) actuallyLogin(w http.ResponseWriter, r *http.Request, login *loginForm) {
	...
	if err != nil {
		s.writeAPIError(w, core.NewError("login error: %v", err))
		return
	}
