...
func (c *Core) dbTrackers(dc *dexConnection) (map[order.OrderID]*trackedTrade, error) {
	...
	unknownCanels := make(map[order.OrderID]struct{})
	for _, dbOrder := range dbOrders {
		if dbOrder.Order.Type() == order.CancelOrderType {
			unknownCanels[dbOrder.Order.ID()] = struct{}{}
		}
	}
	...
	for _, dbOrder := range dbOrders {
		...
		delete(unknownCanels, cancelID) // this one is known
