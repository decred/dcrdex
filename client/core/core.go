...
func (c *Core) ToggleWalletStatus(pw []byte, assetID uint32, disable bool) error {
	...
	if disable {
		...
		if c.assetHasActiveOrders(assetID) {
			return fmt.Errorf("cannot disable %s wallet with active swap negotiations", unbip(assetID))
		}
