...
func (c *Core) dbTrackers(dc *dexConnection) (map[order.OrderID]*trackedTrade, error) {
	...
	cancels := make(map[order.OrderID]struct{})
	trackers := make(map[order.OrderID]*trackedTrade, len(dbOrders))
	for _, dbOrder := range dbOrders {
		ord := dbOrder.Order
		oid := ord.ID()
		// Cancel orders are loaded for target orders. Record it here to make
		// sure we account for all loaded cancel orders.
