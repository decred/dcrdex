...
ETH_SWAP_CONTRACT_HASH_V1=$("${HARNESS_DIR}/alpha" "--preload ${HARNESS_DIR}/deploy.js --exec deploy(\"${ALPHA_ADDRESS}\",\"${ETH_SWAP_V1}\")" | sed 's/"//g')
...
echo "ETH SWAP v0 contract address is ${ETH_SWAP_CONTRACT_ADDR_V0}. Saving to ${NODES_ROOT}/eth_swap_contract_address_v0.txt"
...
echo "ETH SWAP v1 contract address is ${ETH_SWAP_CONTRACT_ADDR_V1}. Saving to ${NODES_ROOT}/eth_swap_contract_address_v0.txt"
...
echo "Deploying ERC20SwapV1 contract for USDC."
USDC_SWAP_CONTRACT_HASH_V1=$("${HARNESS_DIR}/alpha" "--preload ${HARNESS_DIR}/deploy.js --exec deployERC20Swap(\"${ALPHA_ADDRESS}\",\"${ERC20_SWAP_V1}\",\"${TEST_USDC_CONTRACT_ADDR}\")" | sed 's/"//g')
...
echo "USDC SWAP v0 contract address is ${USDC_SWAP_CONTRACT_ADDR_V0}. Saving to ${NODES_ROOT}/usdc_swap_contract_address_v0.txt"
cat > "${NODES_ROOT}/usdc_swap_contract_address_v0.txt" <<EOF
${USDC_SWAP_CONTRACT_ADDR_V0}
EOF

USDC_SWAP_CONTRACT_ADDR_V1=$("${HARNESS_DIR}/alpha" "--preload ${HARNESS_DIR}/contractAddress.js --exec contractAddress(\"${USDC_SWAP_CONTRACT_HASH_V1}\")" | sed 's/"//g')
echo "USDC SWAP v1 contract address is ${USDC_SWAP_CONTRACT_ADDR_V1}. Saving to ${NODES_ROOT}/usdc_swap_contract_address_v1.txt"
