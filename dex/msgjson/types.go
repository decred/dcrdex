...
// The caller must sort orders by rate ascending within each side before
// calling Serialize to ensure deterministic output for signature
// verification.
func (s *MMEpochSnapshot) Serialize() []byte {
	nBuys := len(s.BuyOrders)
	nSells := len(s.SellOrders)
