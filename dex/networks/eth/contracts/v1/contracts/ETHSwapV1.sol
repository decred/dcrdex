...
contract ETHSwap is IAccount, ReentrancyGuard {
    ...
    function initiate(address token, Vector[] calldata vectors)
        ...
        nonReentrant
    {
        require(vectors.length <= MAX_BATCH, "batch too large");
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < reds.length; i++) {
            ...
            // Must match redeemAA's check: blockNum > 0 && blockNum < block.number
            // Also check not already redeemed (secretValidates would be true)
            if (blockNum == 0 || blockNum >= block.number || secretValidates(record, r.v.secretHash)) {
