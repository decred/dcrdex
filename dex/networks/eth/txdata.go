...
func ParseInitiateDataV1(calldata []byte) (map[[SecretHashSize]byte]*SwapVector, error) {
...
func ParseRedeemDataV1(calldata []byte) (map[[SecretHashSize]byte]*RedemptionV1, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expected first arg of type []swapv1.ETHSwapRedemption but got %T", args[0].value)
	}
