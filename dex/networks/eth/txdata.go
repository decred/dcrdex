...
// ParseV0_6HandleOpsData parses the calldata used to call the handleOps function of a
// version 0.6 entrypoint.
func ParseV0_6HandleOpsData(calldata []byte) ([]v6EP.UserOperation, error) {
	abi, err := v6EP.EntrypointMetaData.GetAbi()
	if err != nil {
	...
	toReturn := make([]v6EP.UserOperation, len(userOps))
	for i, userOp := range userOps {
		toReturn[i] = v6EP.UserOperation{
...
// HashV0_6UserOp hashes a user operation for a version 0.6 entrypoint.
func HashV0_6UserOp(op v6EP.UserOperation, epAddress common.Address, chainID *big.Int) (common.Hash, error) {
