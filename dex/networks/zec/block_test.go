...
var (
	//go:embed simnet_block_header.dat
	simnetBlockHeader []byte
	//go:embed block_1624455.dat
	block1624455 []byte
	//go:embed solution_1624455.dat
...
func TestBlock(t *testing.T) {
	...
	const expVersion = 4
	expPrevBlock := mustDecodeHex("000000000136717e394d8de1f86257724bb463348993a1fbb651bd3fd3f1a279")
	...
	if !bytes.Equal(expPrevBlock, hdr.PrevBlock[:]) {
		t.Fatal("wrong previous block", expPrevBlock, hdr.PrevBlock[:])
	}

	if !bytes.Equal(expMerkleRoot, hdr.MerkleRoot[:]) {
