...
type IndexCfg struct {
	...
	// can only be used if this index was created when the table was initially
	// created. If F is non-nil, FBytes must be nil.
...
type TableCfg struct {
	// Indexes are the indexes that are created for the table. To update an
	// index, simple change the name of the index. The old index will be
	...
	err = db.Update(func(txn *badger.Txn) error {
		...
		if err != nil {
			...
			err = cfgItem.Value(func(b []byte) error {
				...
				return oldCfg.UnmarshalBinary(b)
			})
			if err != nil {
				return err
			}
		...
		for indexName := range oldCfg.Indexes {
		...
		for indexName, indexCfg := range cfg.Indexes {
			...
			// If the index is new, we need to pass a non-nil txn to addIndex
			// in order to add all the current table entries to the index.
			var updateTxn *badger.Txn
			if _, exists := oldCfg.Indexes[indexName]; !exists && !newTable {
				updateTxn = txn
			}
