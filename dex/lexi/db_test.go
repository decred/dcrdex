...
func TestTableIndexManagement(t *testing.T) {
	...
	// Test 3: Update existing table - upgrade index version (requires FBytes)
	t.Run("UpgradeIndexVersion", func(t *testing.T) {
		...
		// Verify index still works after upgrade and returns values in reverse order
		count := 0
		...
		if !reflect.DeepEqual(actualOrder, expectedOrder) {
			t.Fatalf("Index upgrade failed: expected order %v, got %v", expectedOrder, actualOrder)
		}
	})
	...
	t.Run("ComplexIndexChanges", func(t *testing.T) {
		...
		// Verify both remaining indexes work
		count := 0
		...
		if !reflect.DeepEqual(actualUpgradeOrder, expectedUpgradeOrder) {
			t.Fatalf("Index upgrade failed: expected order %v, got %v", expectedUpgradeOrder, actualUpgradeOrder)
		}
