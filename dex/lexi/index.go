...
func (db *DB) indexPrefix(tableName, indexName string) (keyPrefix, error) {
	p, err := db.prefixForName(tableName + "__idx__" + indexName)
	if err != nil {
		return keyPrefix{}, err
	}
	return p, nil
}

func (t *Table) deleteIndex(txn *badger.Txn, indexName string) error {
	p, err := t.indexPrefix(t.name, indexName)
	if err != nil {
		return err
	}
...
// indexTable adds all the table entries to the index.
func (t *Table) indexTable(txn *badger.Txn, idx *Index) error {
	if idx.fBytes == nil {
		return fmt.Errorf("cannot index a table with a nil fBytes")
	}
...
func (t *Table) addIndex(name string, fBytes func(k, v []byte) ([]byte, error), f func(k, v KV) ([]byte, error), unique bool, updateTxn *badger.Txn) error {
	p, err := t.indexPrefix(t.name, name)
	if err != nil {
		return err
	}
	...
	if updateTxn != nil {
		err = t.indexTable(updateTxn, idx)
		if err != nil {
