...
func TestNewMarketInfo(t *testing.T) {
	// ok
	mktInfo, err := NewMarketInfo(AssetDCR, AssetBTC, LotSize, LotLimitCoefficient, RateStep, EpochDuration, MarketBuyBuffer)
	if err != nil {
	...
	// bad base
	_, err = NewMarketInfo(8765678, AssetBTC, LotSize, LotLimitCoefficient, RateStep, EpochDuration, MarketBuyBuffer)
	if err == nil {
		t.Errorf("NewMarketInfoFromSymbols succeeded for non-existent base asset")
	}

	// bad quote
	_, err = NewMarketInfo(AssetDCR, 8765678, LotSize, LotLimitCoefficient, RateStep, EpochDuration, MarketBuyBuffer)
	if err == nil {
...
func TestNewMarketInfoFromSymbols(t *testing.T) {
	// ok
	mktInfo, err := NewMarketInfoFromSymbols("dcr", "btc", LotSize, LotLimitCoefficient, RateStep, EpochDuration, MarketBuyBuffer)
	if err != nil {
		t.Errorf("NewMarketInfoFromSymbols failed: %v", err)
	}
	if mktInfo.Name != "dcr_btc" {
	...
	// bad base
	_, err = NewMarketInfoFromSymbols("super fake asset", "btc", LotSize, LotLimitCoefficient, RateStep, EpochDuration, MarketBuyBuffer)
	if err == nil {
		t.Errorf("NewMarketInfoFromSymbols succeeded for non-existent base asset")
	}

	// bad quote
	_, err = NewMarketInfoFromSymbols("dcr", "btc not BTC or bTC", LotSize, LotLimitCoefficient, RateStep, EpochDuration, MarketBuyBuffer)
	if err == nil {
		t.Errorf("NewMarketInfoFromSymbols succeeded for non-existent quote asset")
	}
}
